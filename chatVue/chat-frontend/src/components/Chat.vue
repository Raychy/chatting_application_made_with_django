...
export default {
  ...
  methods: {
	    startChatSession () {
	     $.post('http://localhost:8081/api/chats/', (data) => {
	  ...
	  postMessage (event) {
	      ...
	      $.post(`http://localhost:8081/api/chats/${this.$route.params.uri}/messages/`, data, (data) => {
	    ...
	    joinChatSession () {
	      ...
	      $.ajax({
	        url: `http://localhost:8081/api/chats/${uri}/`,
	    ...
	    fetchChatSessionHistory () {
	      $.get(`http://127.0.0.1:8081/api/chats/${this.$route.params.uri}/messages/`, (data) => {
